...
MAX_LENGTH = 128
TEST_PROMPTS = [
    "### Instruction: Viết một đoạn văn ngắn về Hà Nội\n### Input: \n### Response: ",
    "### Instruction: Giải thích tại sao phở là món ăn nổi tiếng ở Việt Nam\n### Input: \n### Response: ",
    "### Instruction: Kể một câu chuyện ngắn về lịch sử Việt Nam\n### Input: \n### Response: ",
]
...
class AlpacaDataset(Dataset):
    """Custom dataset for Alpaca Cleaned Dataset on HuggingFace"""
    ...
    def __getitem__(self, idx):
        ...
        # Kết hợp instruction, input (nếu có), và output
        instruction = item['instruction']
        input_text = item['input'] if item['input'] else ""
        output_text = item['output']
        
        # Tạo prompt theo định dạng phù hợp (tùy thuộc vào cách bạn muốn định dạng)
...
def filter_function(example, config):
    ...
    return (
        ...
        len(example['output'].strip()) > config.min_text_length
    )

def load_n_preprocess_data(config):
    """Download Alpaca Cleaned dataset from HuggingFace and preprocess"""
    # Download dataset
...
def create_train_valid_set(dataset, tokenizer, config):
    ...
    train_ds = AlpacaDataset(train_split, tokenizer, config.max_length)
    valid_ds = AlpacaDataset(valid_split, tokenizer, config.max_length)
